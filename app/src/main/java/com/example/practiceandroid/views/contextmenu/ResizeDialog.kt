...
fun validateWidth(width: String): Boolean {
    val widthValue = width.toFloatOrNull()
    return widthValue != null && widthValue in 1f..30f
}

// Функция для проверки высоты
fun validateHeight(height: String): Boolean {
    val heightValue = height.toFloatOrNull()
    return heightValue != null && heightValue in 1f..30f
}
            ...
            Column(
                ...
                OutlinedTextField(
                    ...
                    onValueChange = {
                        val filteredText = it.filter { it.isDigit() || it == '.' }
                        newWidth = filteredText
                        widthError = !validateWidth(newWidth)
                    },
                    label = { Text("Ширина (мин =  50)") },
                    keyboardOptions = KeyboardOptions.Default.copy(keyboardType = KeyboardType.Number),
                    isError = widthError,
                    colors = TextFieldDefaults.outlinedTextFieldColors(
                        errorBorderColor = Color.Red
                ...
                OutlinedTextField(
                    ...
                    onValueChange = {
                        val filteredText = it.filter { it.isDigit() || it == '.' }
                        newHeight = filteredText
                        heightError = !validateHeight(newHeight)
                    },
                    label = { Text("Высота (мин = 50)") },
                    keyboardOptions = KeyboardOptions.Default.copy(keyboardType = KeyboardType.Number),
                    isError = heightError,
                    colors = TextFieldDefaults.outlinedTextFieldColors(
                        errorBorderColor = Color.Red
